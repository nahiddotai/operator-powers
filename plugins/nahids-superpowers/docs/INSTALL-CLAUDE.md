...
3. Review what the installer shows you: the plugin ships skills, three hooks (session notice, discovery hints, submission guard), and one update server (MCP) server. All hook source is in `hook-runner/` in this repository.
4. Start a new session. You'll see a one-time welcome line. Say the job you want done, or run `/operator-superpowers:start-here`.
