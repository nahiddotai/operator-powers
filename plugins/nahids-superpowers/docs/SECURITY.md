...
- **Server-side enforcement.** The update server independently validates payload shape, field allowlist, size caps, token signature, and expiry. The client-side guard is a second line, not the only line.
