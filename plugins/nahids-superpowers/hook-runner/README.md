...
Deterministic, dependency-free Node script behind the plugin's hooks. No shell execution, no dynamic code loading, no transcript access. Its one network action is the anonymous usage ping (fixed six-field payload, off switch, see docs/PRIVACY.md), sent from a detached child process so hooks never wait on the network. Reads stdin, the bundled read-only catalogue, and `~/.nahids-superpowers/state.json`; writes only that state file.
