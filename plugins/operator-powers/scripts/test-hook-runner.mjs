...
function run(cmd, stdinObj, home, extraEnv = {}) {
  const res = spawnSync(process.execPath, [RUNNER, cmd], {
    ...
    env: { ...process.env, HOME: home, OPERATOR_POWERS_TELEMETRY_URL: "", ...extraEnv },
    encoding: "utf8",
    timeout: 5000,
  });
...
{
  const home = freshHome();
  mkdirSync(join(home, ".operator-powers"), { recursive: true });
...
// --- discover ---
{
  ...
  const hintCount = (multi.out.match(/installed power/g) || []).length;
...
// --- guard-mcp-write ---
{
  ...
  check("read tool passes through", run("guard-mcp-write", { tool_name: "mcp__operator_powers__search_powers", tool_input: { query: "x" } }, home).out === "");
  check("missing token denied", run("guard-mcp-write", { tool_name: "mcp__operator_powers__submit_feedback", tool_input: { payload } }, home).out.includes("deny"));
  check("hash mismatch denied", run("guard-mcp-write", { tool_name: "mcp__operator_powers__submit_feedback", tool_input: { payload: { ...payload, note: "changed after preparation" }, payloadHash: goodHash, confirmationToken: "t" } }, home).out.includes("deny"));
...
// --- telemetry ---
{
  ...
  const T = { OPERATOR_POWERS_TELEMETRY_URL: `http://127.0.0.1:${server.address().port}/t` };
  ...
  run("session-start", {}, home2, { ...T, OPERATOR_POWERS_NO_TELEMETRY: "1" });
  await new Promise((r) => setTimeout(r, 600));
